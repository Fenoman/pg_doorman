...
При положительном `query_wait_timeout` единый дедлайн отсчитывается от
входа в `timeout_get` и ограничивает ожидание PAUSE, семафора, возврата
соединения, ограничителя всплесков, проверки пригодности и координатора,
включая выдачу резерва. Переход между фазами не запускает бюджет заново.
Когда создание backend уже разрешено, подключение получает отдельный
`connect_timeout` и может завершиться после дедлайна ожидания запроса.
...
с таймаутами `reserve_pool_timeout` (по умолчанию 3000 ms) и выдачи
резерва. Ноль не устанавливает общий дедлайн выдачи соединения.
Чтобы ограничить суммарное ожидание во всех фазах при включённом
координаторе, задайте положительный `query_wait_timeout`.
...
Если получатель отменён после доставки, backend передаётся следующему
ожидающему либо возвращается в idle. Повторного возврата semaphore
permit при такой передаче нет.
...
При помещении backend в очередь свободных соединений координатор
(если настроен) уведомляется о возврате, чтобы ожидающие Фазы C из
соседних пулов могли просканировать кандидатов для выселения.
Ожидающие внутри того же пула получают
...
При `max_db_connections = 0` (по умолчанию) общий лимит на базу отключён.
Координатор продолжает учитывать соединения, чтобы `RELOAD` мог включить единый
лимит для всех поколений пулов. Все описанные выше механизмы независимых пулов
продолжают работать. Новое соединение получает разрешение под короткой блокировкой
бюджета; при обычном переиспользовании idle проверяется только атомарный признак
необходимости сокращения. После уменьшения лимита лишние idle-соединения закрываются
до следующей выдачи. См. [контракт RELOAD](../reference/pool.md#max_db_connections).
...
Пока вызов ждёт координатора, он также стоит в очереди прямой передачи
своего пула. Вернувшийся backend сохраняет свой permit базы данных и
может сразу обслужить ожидающий запрос даже при полном лимите БД.
Успешная передача отменяет запрос нового permit к координатору.
Положительный дедлайн ожидания запроса ограничивает оба варианта и
последующее ожидание повторного захвата ограничителя всплесков.
...
```
   ...
   Фаза 5: ограничитель всплесков (scaling_max_parallel_creates)
              ...
              v
   +-------------------------------+
   | JIT-захват координатора       |  учёт create даже при выключенном лимите
   |  fast: try_acquire()          |  короткая блокировка бюджета
...
работает **внутри** слота ограничителя всплесков. При `max_db_connections = 0`
он учитывает соединение, не ограничивая общий бюджет базы.

### Когда координатор настроен, но лимит не достигнут

Если `max_db_connections = 80`, а текущее использование 30, фаза A
координатора успешна, пока сохраняется свободный бюджет. Фазы резервирования,
вытеснения и ожидания не запускаются. Для выдачи нового соединения берётся
короткая блокировка бюджета; при обычном переиспользовании idle проверяется
атомарный признак необходимости сокращения без этой блокировки.
...
| `max_db_connections` | не задан (выключено) | пул | Лимит суммарного числа серверных соединений к базе по всем пользовательским пулам и поколениям. Когда не задан, общий бюджет базы не ограничен. |
