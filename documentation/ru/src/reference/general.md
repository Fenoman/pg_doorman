...
Размер пачки в релее «бэкенд → клиент». pg_doorman накапливает сообщения DataRow и CopyData
в буфере, привязанном к бэкенду, и отдаёт пачку клиенту, как только буфер достигает этого
порога, — поэтому крупная выборка доезжает примерно за `размер_ответа / порог` циклов записи.
Увеличение значения с 8 KB до 64 KB сокращает число итераций `recv`/`write` на отчёте в 20 MB
с ~2440 до ~305.

**На OLTP-нагрузку этот параметр не влияет.** Ответ меньше порога уходит клиенту по приходу
`ReadyForQuery` и до проверки порога не доходит, так что число записей на коротких запросах
не зависит от значения. Поднимать его имеет смысл только на нагрузках с крупными выборками
(отчёты, выгрузки, `COPY TO`).

Цена — память: бэкенду, чей буфер перерос порог, выдаётся замена размером в два порога, и она
живёт до закрытия соединения. При значении по умолчанию 64 KB это до 128 KB на каждый бэкенд,
обслуживший крупный ответ; считать нужно по числу бэкендов, а не клиентов.

Допустимый диапазон: 4096 – 1048576 байт. Ниже 4 KB ответы дробятся так мелко, что число
syscall'ов перевешивает выигрыш от батчинга; выше 1 MB экономия уже насыщена.

По умолчанию: `65536 (64 KB)`.
...
Максимальное время ожидания завершения активных транзакций при плавном завершении или горячей замене процесса через `SIGINT` или `SIGUSR2`. Зависимость поведения `SIGINT` от режима запуска описана в разделе [Сигналы](../operations/signals.md).

`SIGTERM` завершает процесс немедленно и не ждёт `shutdown_timeout`.

По умолчанию: `10000 (10 sec)`.

### proxy_copy_data_timeout

Максимальная пауза без успешного чтения или записи байтов при проксировании, в миллисекундах.
Применяется к потоковой передаче тел DataRow, CopyData и FunctionCallResponse, записи ответов
и чтению частично поступивших внеочередных сообщений PostgreSQL. Каждое успешное чтение или
запись начинает отсчёт заново: передача с непрерывным прогрессом может длиться дольше этого значения.
Отдельная операция flush ограничена тем же временем, поскольку интерфейс потока не сообщает
о частичном прогрессе flush.
