...
`RELOAD` меняет общий бюджет для всех прежних и новых пользовательских пулов,
включая включение и отключение лимита через `0`. Текущие выдачи завершаются штатно.
Если новый `max_db_connections + reserve_pool_size` ниже занятого бюджета,
лишние idle-соединения закрываются, а активные соединения выводятся из пула по мере
возврата. Это административное уменьшение имеет приоритет над возрастом соединений
и минимумами пользователей. Новое соединение не создаётся, пока в общем бюджете
не освободится место. Уже закрываемые соединения могут кратковременно оставаться
видимыми в PostgreSQL до завершения асинхронного закрытия; клиентам они не выдаются.
