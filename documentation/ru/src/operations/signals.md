...
- Таймауты жизненного цикла соединений — `general.idle_timeout`,
  `general.server_lifetime`, `general.server_idle_check_timeout`,
  `general.query_wait_timeout`, `general.connect_timeout`. Пул фиксирует их
  при создании, поэтому каждый пул, наследующий изменённое значение,
  **пересоздаётся**; пулы с собственным переопределением на уровне пула не
  трогаются. Сессии, которые ещё держат прежнее поколение, сохраняют его до
  отключения — поколение дренажится в фоне, а не закрывается под ними, так
  что PostgreSQL какое-то время несёт бэкенды обоих поколений. Имена
  изменённых полей пишутся в лог на уровне `info`.
...
- Остальные значения `general`, которые пул фиксирует при создании:
  ...
  `fallback_lifetime`. Их применение означало бы пересоздание всех пулов, а
  оно стоит холодного кеша prepared statements и всплеска повторных `Parse`
  к PostgreSQL — для размера кешей и настроек failover цена не оправдана.
  Пулы продолжают работать со старыми значениями до рестарта процесса; в
  лог пишется `warn` с перечислением изменённых полей.
