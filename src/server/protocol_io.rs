    ...
    C: tokio::io::AsyncWrite + std::marker::Unpin,
{
    ...
    loop {
        ...
        if non_streamable_message_exceeds_config_cap(code_u8, message_len, server.max_message_size)
        {
            error!(
                "[{}@{}] non-streamable message size limit exceeded pid={}: type={} received={} bytes, max={} bytes",
                server.address.username,
                server.address.pool_name,
                server.get_process_id(),
                code_u8 as char,
            ...
            server.mark_bad(
                format!(
                    "Non-streamable message size limit exceeded: type={} len={} bytes (max: {} bytes)",
                    code_u8 as char, message_len, server.max_message_size
                )
                .as_str(),
            );
