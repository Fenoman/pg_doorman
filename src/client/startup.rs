    ...
    pub async fn startup(
        ...
        );
        {
            ...
            if !admin && username_from_parameters == TALOS_USERNAME && hba_ok {
                ...
                let talos_role = talos_role_to_string(token.role);
                let (role_hba_md5, role_hba_scram) =
                    talos_role_hba_checks(&transport, &talos_role, &pool_name);
                if !talos_hba_allowed(role_hba_md5, role_hba_scram) {
                    error_response_terminal(
                        &mut write,
                        "Talos role is not allowed by pg_hba rules.",
                        "28000",
                    )
                    .await?;
                    return Err(Error::AuthError(format!(
                        "Talos role {talos_role} is not allowed by pg_hba for database {pool_name}"
                    )));
                }
                client_identifier.application_name = token.client_id;
                client_identifier.username = talos_role;
                client_identifier.hba_md5 = role_hba_md5;
                client_identifier.hba_scram = role_hba_scram;
