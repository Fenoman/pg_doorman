    ...
    T: tokio::io::AsyncWrite + std::marker::Unpin,
{
        ...
        if contains_discard_all(parse.query().as_bytes()) {
            ...
            if client_given_name.is_empty() {
                if let Some(hash) = self.prepared.last_anonymous_hash.take() {
                    self.prepared
                        .cache
                        .pop(&PreparedStatementKey::Anonymous(hash));
                }
            } else {
                self.prepared
                    .cache
                    .pop(&PreparedStatementKey::Named(client_given_name.clone()));
            }
    ...
    ///
    /// Uses split_off/unsplit to avoid a full buffer copy when ReadyForQuery is at the end.
    #[inline(always)]
    pub(crate) fn inject_discard_responses(&mut self, response: &mut BytesMut) {
        ...
        if let Some(offset) = ready_offset {
            // Split off the trailing ReadyForQuery, append discard responses, reattach tail.
            // split_off/unsplit reuse the existing allocation when memory is contiguous.
            let tail = response.split_off(offset);
            response.put(&self.discard_response_buffer[..]);
            response.unsplit(tail);
        } else {
            response.put(&self.discard_response_buffer[..]);
        }
    ...
    fn try_filter_close(&mut self, close: &Close) -> bool {
        ...
        if close.is_prepared_statement() {
            ...
            if close.anonymous() {
                ...
                self.prepared
                    .cache
                    .pop(&PreparedStatementKey::Named(close.name.clone()));
            }
        } else {
            if close.name.is_empty() {
                self.filter_unnamed_prepared_statement = false;
